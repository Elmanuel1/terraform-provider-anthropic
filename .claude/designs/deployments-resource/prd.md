...
- Modeling non-text seed events (image/document/outcome/system) — deferred.
- Modeling mounted `resources` (repositories / files / memory stores) — deferred.
...
- Manual run trigger, non-text seed events, and mounted `resources` — each deferred; the
